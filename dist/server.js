...
// node_modules/fastify-jwt/index.js
var require_fastify_jwt = __commonJS({
  "node_modules/fastify-jwt/index.js"(exports2, module2) {
    "use strict";
...
async function RegisterNewLeader(app2) {
  app2.post("/register/leader", { preHandler: app2.authenticate }, async (req, reply) => {
...
async function RegisterNewCollaborator(app2) {
  app2.post("/register/collaborator", { preHandler: app2.authenticate }, async (req, reply) => {
    const collaboratorSchema = import_zod2.z.object({
      name: import_zod2.z.string(),
      salary: import_zod2.z.number(),
      jobId: import_zod2.z.string().uuid(),
      leaderId: import_zod2.z.string().uuid(),
      influence: import_zod2.z.number()
    });
...
async function GetAllLeaders(app2) {
  app2.get("/leaders", { preHandler: app2.authenticate }, async () => {
...
async function GetAllCollaborators(app2) {
  app2.get("/collaborators", { preHandler: app2.authenticate }, async () => {
    const collaborators = await prisma.collaborator.findMany();
...
var fastifyJWT = require_fastify_jwt();
var jwt = require("jsonwebtoken");
function Auth(app2, jwtSecret2) {
  app2.register(fastifyJWT, { secret: jwtSecret2 });
  app2.decorate("authenticate", async (req, reply) => {
    try {
      await req.jwtVerify();
    } catch (err) {
      reply.status(401).send({ error: "Token inv\xE1lido ou expirado" });
    }
  });
  app2.post("/login", async (req, reply) => {
    ...
    try {
      ...
      if (!user) {
        return reply.status(401).send({ error: "Usu\xE1rio n\xE3o encontrado" });
      }
      if (user.password != password) {
        return reply.status(401).send({ error: "Senha inv\xE1lida" });
      }
      const token = jwt.sign({ username }, jwtSecret2, { expiresIn: "5h" });
      reply.send({ token });
    } catch (error) {
      return reply.status(400).send({ error: "Dados inv\xE1lidos" });
    }
  });
}

// src/routes/Leader/sort-leaders-by-influence.ts
async function SortLeadersByInfluence(app2) {
  app2.get("/leaders/influence", { preHandler: app2.authenticate }, async () => {
    const leaders = await prisma.leader.findMany({
      orderBy: {
...
// src/routes/Positions/get-all-job-positions.ts
async function GetAllJobPositions(app2) {
  app2.get("/jobPositions", { preHandler: app2.authenticate }, async () => {
...
async function GetAllLeadershipPositions(app2) {
  app2.get("/leadershipPositions", { preHandler: app2.authenticate }, async () => {
...
async function CheckLeaderName(app2) {
  app2.get("/leader/:name", { preHandler: app2.authenticate }, async (req) => {
...
// src/routes/Collaborator/check-collaborator-name.ts
var import_zod5 = require("zod");
async function CheckCollaboratorName(app2) {
  app2.get("/collaborator/:name", { preHandler: app2.authenticate }, async (req) => {
...
async function RemoveCollaborators(app2) {
  app2.put("/remover/colaborador", { preHandler: app2.authenticate }, async (req, reply) => {
    const collaboratorSchema = import_zod6.z.object({
      ids: import_zod6.z.array(import_zod6.z.string().uuid())
    });
...
async function RemoveLeaders(app2) {
  app2.put("/remover/lideranca", { preHandler: app2.authenticate }, async (req, reply) => {
    const leaderSchema = import_zod7.z.object({
      ids: import_zod7.z.array(import_zod7.z.string().uuid()),
...
async function CreateNewLeadershipPosition(app2) {
  app2.post("/leaders/novo-cargo", { preHandler: app2.authenticate }, async (req, reply) => {
    const newLeadershipPositionSchema = import_zod8.z.object({
      nameOfPosition: import_zod8.z.string(),
      weightOfThePosition: import_zod8.z.number()
    });
...
async function CreateNewJobPosition(app2) {
  app2.post("/collaborators/novo-cargo", { preHandler: app2.authenticate }, async (req, reply) => {
    const newJobPositionSchema = import_zod9.z.object({
      nameOfPosition: import_zod9.z.string(),
      weightOfThePosition: import_zod9.z.number()
    });
